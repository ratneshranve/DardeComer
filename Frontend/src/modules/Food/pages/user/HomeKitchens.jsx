...
import { ArrowLeft, Clock, MapPin, Heart, Star, Home } from "lucide-react"
...
export default function HomeKitchens() {
        ...
        const transformed = list
          .map((restaurant) => {
            ...
            return {
              ...
              slug,
              name: restaurant?.name || "Unknown Home Kitchen",
              cuisine,
          ...
          .filter((restaurant) => restaurant.vendorType === "home_kitchen")

        setHomeKitchens(transformed)
      } catch (error) {
        if (!cancelled) {
          setHomeKitchens([])
        }
  ...
  return (
    <AnimatedPage className="min-h-screen bg-gradient-to-b from-blue-50/20 dark:from-[#0a0a0a] via-white dark:via-[#0a0a0a] to-orange-50/20 dark:to-[#0a0a0a]">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 xl:px-12 py-4 sm:py-6 md:py-8 lg:py-10 space-y-4 sm:space-y-6 lg:space-y-8">
        ...
        {showRestaurantsSkeleton ? (
          ...
          <div className="py-16 text-center text-sm text-gray-500">
            No home kitchens found in your selected location right now.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-5 xl:gap-6 pt-2 sm:pt-3 lg:pt-4">
            {homeKitchens.map((restaurant, index) => {
                    ...
                    <Card className="overflow-hidden cursor-pointer border border-gray-200 dark:border-gray-800 group bg-white dark:bg-[#1a1a1a] hover:shadow-lg dark:hover:shadow-xl dark:hover:shadow-gray-900/50 pb-1 sm:pb-2 lg:pb-3 flex flex-col h-full w-full transition-all duration-300">
                      <div className="flex flex-row min-h-[120px] sm:min-h-[140px] md:min-h-[160px] lg:min-h-[180px] flex-1">
                            ...
                            <div className="flex-shrink-0">
                              <div className="flex items-start justify-between gap-2 mb-2">
                                <div className="flex-1 min-w-0 pr-2">
                                  ...
                                  <span className="inline-flex items-center rounded-full border border-blue-200 dark:border-blue-700 bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 text-[10px] font-semibold text-blue-700 dark:text-blue-200 mb-2">
                                    Home Kitchen
                                  </span>
                        ...
                        <div className="w-36 sm:w-44 md:w-56 lg:w-64 xl:w-72 flex-shrink-0 relative overflow-hidden group/image">
                          <img
                            src={restaurant.image || "https://via.placeholder.com/400x300?text=Home+Kitchen"}
                            alt={restaurant.name}
