...
export default function BottomNavigation() {
  ...
  const TabIcon = (active, Outline, Solid) => {
    ...
    return <Icon className={`w-6 h-6 ${active ? "text-primary" : "text-gray-500"}`} />
  }
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50 pb-[env(safe-area-inset-bottom)]">
          ...
          {profileImage && !imageError ? (
            <img
              ...
              className={`w-7 h-7 rounded-full border-2 object-cover ${
                isActive("/delivery/profile") ? "border-primary" : "border-gray-300"
              }`}
              onError={() => {
                setImageError(true)
              }}
