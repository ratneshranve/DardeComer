...
export default function DeliveryHomeV2({ tab = 'feed' }) {
  ...
  return (
    <div className="relative h-screen w-full bg-white text-gray-900 overflow-hidden flex flex-col">
      ...
      {/* ─── 2. MAIN CONTENT ─── */}
      <div className={`flex-1 relative overflow-y-auto ${currentTab === 'history' ? 'pt-0' : 'pt-[calc(env(safe-area-inset-top,0px)+144px)]'} no-scrollbar`}>
         {currentTab === 'feed' ? (
           <div className="absolute inset-0 top-[calc((env(safe-area-inset-top,0px)+144px)*-1)]">
             <LiveMap 
             ...
             {isSimMode && (
               <div className="absolute top-[calc(env(safe-area-inset-top,0px)+234px)] left-4 right-4 z-[100] bg-black/80 backdrop-blur-md rounded-xl p-4 border border-white/20 flex items-center justify-between shadow-2xl">
                  <div className="flex items-center gap-4">
            ...
            >
              <div className="w-full pointer-events-auto relative">
                {incomingOrder && (
                  <NewOrderModal 
                    order={incomingOrder} 
                    onAccept={(o) => { stopNotificationSound(); acceptOrder(o); setIncomingOrder(null); clearNewOrder(); }}
                    onReject={() => { stopNotificationSound(); setIncomingOrder(null); clearNewOrder(); }}
