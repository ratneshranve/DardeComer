...
export const verifyUserOtpAndLogin = async (
  ...
  if (!normalized || normalized.length !== 10) {
    throw new ValidationError("Invalid phone number. 10 digits required.");
  }
  ...
  return { accessToken, refreshToken, user: { ...user, role }, role, isNewUser };
};
...
export const logout = async (refreshToken, fcmToken, platform = "web") => {
  ...
  // 2. Invalidate the refresh token if provided
